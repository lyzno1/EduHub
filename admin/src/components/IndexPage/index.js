...
const IndexPage = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [appsResponse, configResponse] = await Promise.all([
                ...
                fetch(`${API_URL}/api/student-config`) 
            ]);
            ...
            console.error("Error fetching data for StudentPage:", error); 
            message.error(`加载学生端数据失败: ${error.message}`);
    ...
    const handleAppCheckChange = (folderKey, isChecked) => {
        setAllowedApps(prev => {
            ...
            if (isChecked) {
                ...
                // When checking an app, preserve existing order of its cards if any, or add all if none were selected.
                ...
                newState[folderKey] = [...existingCardsInOrder, ...newCardsToAdd];
                 // If all cards are now selected, ensure their order matches availableAppsData if no prior order existed
    ...
    const handleSave = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_URL}/api/update-student-config`, { 
                method: 'POST',
                headers: {
    ...
    return (
        <Spin spinning={loading}>
            <Card title="学生可见应用与功能卡片配置">
                <Typography.Paragraph type="secondary">
                    勾选应用或其下的具体功能卡片，以控制学生用户界面中可见的内容。使用箭头调整显示顺序。保存后生效。
